<template>
  <NuxtLayout name="admin">
    ...
    <div class="space-y-6">
      ...
      <div v-if="adminStore.loading" class="glass-panel rounded-2xl overflow-hidden border border-brand-100/50 shadow-glass relative min-h-[300px]">
        <div class="overflow-x-auto">
          <div class="border-b border-brand-100 bg-brand-100/30 py-4 px-6 flex justify-between animate-pulse">
            <div class="w-1/4 h-3 bg-slate-200/80 rounded"></div>
            <div class="w-1/6 h-3 bg-slate-200/80 rounded"></div>
            <div class="w-16 h-3 bg-slate-200/80 rounded"></div>
            <div class="w-16 h-3 bg-slate-200/80 rounded"></div>
            <div class="w-24 h-3 bg-slate-200/80 rounded"></div>
          </div>
          <div class="divide-y divide-brand-100 px-6">
            <div v-for="n in 4" :key="n" class="py-4 flex justify-between items-center animate-pulse">
              <div class="w-1/4 h-3.5 bg-slate-200/60 rounded"></div>
              <div class="w-1/6 h-3 bg-slate-200/50 rounded"></div>
